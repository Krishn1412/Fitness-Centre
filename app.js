    ...
    app.get("/welcome",(req,res)=>{
        db.query("SELECT name,weight,id,height,age,BMI,Address,Time,BuddyID FROM users WHERE email = ?",[username1],(error,results)=>{
            ...
            if(results.length >0){
                ...
                ida=results[0].id;
                console.log(bdd);
                return res.render("e:/Krishn/dbms project/views/DashboardU.hbs",{
...
app.post("/BMI",(req,res)=>{
    ...
    db.query("UPDATE users SET ? WHERE email=?",[{weight: weight, height:height,age:age,BMI:bmi},[email1]],(error,results) =>{
        if(error){
            ...
            res.sendFile("e:/Krishn/dbms project/views/Slot_booking.html")
        }
    }
        );
    ...
    db.query("SELECT id FROM users WHERE  email = ? ",[email1],(error,results)=>{
        ...
        if(results.length >0){
            console.log(results);
             ...
             ida=   results[0].id;
            console.log(ida);
            res.sendFile("e:/Krishn/dbms project/views/BMI.html");
        }
    });
...
app.post("/slot",(req,res)=>{
    ...
    db.query("UPDATE users SET ? WHERE email=?",[{Address: address,Time:time},[email1]],(error,results) =>{
        if(error){
            ...
            res.sendFile("e:/Krishn/dbms project/views/GymBuddy.html")
        }
    }
        );
        ...
        db.query("SELECT id FROM users WHERE  Address = ? and Time=? and BuddyID IS NULL  ",[address1,time1],(error,results)=>{
            ...
            if(results.length >0){
              ...
              if(idB.length>1){
                ...
                console.log(idB[p]);
                bid=idB[p];
                res.sendFile("e:/Krishn/dbms project/views/Buddy1.html");
              }
              else{
                res.sendFile("e:/Krishn/dbms project/views/DenyBuddy1.html");
              }   
            }
        });
    }
    else if(option==='no'){
    res.sendFile("e:/Krishn/dbms project/views/pricing.html");
    }
...
app.post("/bud",(req,res)=>{
        ...
        db.query("SELECT name,id,BMI FROM users WHERE  id = ? ",[bid],(error,results)=>{
            ...
            if(results.length >0){
                console.log(results);
                return res.render("e:/Krishn/dbms project/views/Buddy2.hbs",{
   ...
   const {plan} = req.body;
    ...
    db.query("UPDATE users SET ? WHERE email=?",[{Plan: plan},[email1]],(error,results) =>{
        if(error){
            ...
            res.sendFile("e:/Krishn/dbms project/views/ProfileComplete.html")
        }
    }
        );
...
app.get("/budye",(req,res)=>{
    ...
    if(bdd===null){
        return res.render("e:/Krishn/dbms project/views/buddyUpdate2.hbs");
        }
    else{
        ...
        if(results.length >0){
            console.log(results);
            return res.render("e:/Krishn/dbms project/views/buddyUpdate.hbs",{
                message1:results[0].name,
                message2:results[0].id,
                message3:results[0].BMI,
        ...
        db.query("SELECT id FROM users WHERE  Address = ? and Time=? and BuddyID IS NULL  ",[address1,time1],(error,results)=>{
            ...
            if(results.length >0){
              ...
              if(idB.length>1){
                ...
                console.log(ida);
                bdd=idB[p];
                res.sendFile("e:/Krishn/dbms project/views/buddyUpdate4.html");
              }
              else{
                res.sendFile("e:/Krishn/dbms project/views/DenyBuddy1.html");
              }    
...
app.post("/bud1",(req,res)=>{
    ...
    db.query("SELECT name,id,BMI FROM users WHERE  id = ? ",[bdd],(error,results)=>{
        ...
        if(results.length >0){
            console.log(results);
            return res.render("e:/Krishn/dbms project/views/buddyUpdate3.hbs",{
                message1:results[0].name,
                message2:results[0].id,
                message3:results[0].BMI,
...
app.get("/pland",(req,res)=>{
        ...
        if(results.length >0){
            console.log(results);
            return res.render("e:/Krishn/dbms project/views/PricingUpdate.hbs",{
...
app.post("/pa1",(req,res)=>{
 ...
 app.get("/slotd",(req,res)=>{
        ...
        if(results.length >0){
            console.log(results);
            return res.render("e:/Krishn/dbms project/views/SlotUpdate.hbs",{
...
app.post("/sl1",(req,res)=>{
    ...
    if(option==='yes'){
                res.sendFile("e:/Krishn/dbms project/views/SlotUpdate2.html");
            }
...
app.get("/profiled",(req,res)=>{
        return res.sendFile("e:/Krishn/dbms project/views/profileUpdate.html");
...
app.get("/cp1",(req,res)=>{ 
        db.query("SELECT Comment_text,name,id FROM comments  ",(error,results)=>{
            ...
            if(results.length >0){
                ...
                console.log(itext);
                console.log(iid);
                return res.render("e:/Krishn/dbms project/views/Comments3.hbs",{
                    results:results
                });   
            }
        });
...
app.post("/cp2",(req,res)=>{
    ...
    db.query("INSERT INTO comments (Comment_text,id,name)  VALUES ? ",[records],(error,results)=>{
        if(error){
            ...
            return res.sendFile("e:/Krishn/dbms project/views/Comment2.html");    
        }
    });
...
app.post("/cp4",(req,res)=>{  
    return res.sendFile("e:/Krishn/dbms project/views/Comments1.html");
});
